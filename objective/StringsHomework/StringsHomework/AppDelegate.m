...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
   ...
   //1.1
    if (range.location != NSNotFound) {
        NSString* textFromFirstHalf = [text substringToIndex:range.location];
        NSLog(@"first half : %@", textFromFirstHalf);
    } else {
    ...
    */
    ...
    //1.2
    if (range.location != NSNotFound) {
        NSString* textFromSecondHalf = [text substringFromIndex:range.location];
        NSLog(@"second half : %@", textFromSecondHalf);
    } else {
        NSLog(@"range not found");
    }
    */
...
//    STUDENT
//    4
    //NSMutableArray* mutableArray = [array mutableCopy]; //создание массива на основе другого массива
 
    NSArray* array = [text componentsSeparatedByString:@"."]; //Создание массива из предложений
    NSMutableArray* mutableArray = [[NSMutableArray alloc]init]; // Создание mutable массива
    NSString* stringForMutableArray = [NSString stringWithFormat:@""]; //Создание строки,в которую мы будем класть предложения со словом NSString
    
    for (NSString* string in array) {
        if ([string containsString:@"NSString"]) {
            stringForMutableArray = string;
            [mutableArray addObject:stringForMutableArray];
        }
    }
//    NSLog(@"%@", mutableArray);
    ...
    for (NSString* string in mutableArray) {
        NSArray* reversedWords = [string componentsSeparatedByString:@" "];
        NSArray* reversed = [[reversedWords reverseObjectEnumerator] allObjects];
        NSString* resultString =[reversed componentsJoinedByString:@" "];
//        resultString = [resultString stringByReplacingOccurrencesOfString:@"." withString:@" "];
        [reversedArray addObject:resultString];
    ...
    NSArray* arrayOne = [myStr componentsSeparatedByString:@" " ];
    NSArray* reversedArray = [[arrayOne reverseObjectEnumerator] allObjects];
    NSLog(@"%@",reversedArray);
